...
def numeros_a_letras(numeros):
    ...
    for numero in range(0, len(numeros), 3):
        letra = chr(numeros[numero] + numeros[numero+1])
        mensaje += letra
    return mensaje
...
def letras_a_numeros(texto):
    resultado = []
    for letra in texto:
        numero = ord(letra)  # Calcula el número basado en el orden alfabético
        ...
        for msg in track:
            if msg.type == 'program_change':
                if centinela < len(listanum):
                    if listanum[centinela]>127:
                        outputTrack.append(Message('program_change', program=127, time=0))
                        outputTrack.append(Message('program_change', program=listanum[centinela]%127, time=0))
                    else:
                        outputTrack.append(Message('program_change', program=listanum[centinela], time=0))
                        outputTrack.append(Message('program_change', program=0, time=0))
                    centinela+=1
                else:
                ...
                outputTrack.append(msg)
    outputFile.save('mensajeOcultoEsp.mid')
    print("========================================================================================")
    print("*                                                                                      *")
    print("*  SU MENSAJE HA SIDO OCULTO DE MANERA EXITOSA EN EL ARCHIVO mensajeOcultoEsp.mid      *")
    print("*                                                                                      *")
    print("========================================================================================")
